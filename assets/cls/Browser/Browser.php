 ...
 * Last Modified:		September 17th, 2013
 * @version				2.1
 *
 ...
 *
 * 2013-09-17 (v2.0.1):
 *  + All following modifications will be tracked through Git.
 *
 * 2012-08-15 (v2.0):
...
class Browser {
	...
	*/
	public function reset() {
		...
		$this->_engine_name = self::ENGINE_UNKNOWN;
	...
	*/
	...
	/**
	* Creates an array with simplified browser information
	* @return array Contains the engine, name, and version (add rest later)
	*/
	public function getBrowserArray() {
		$classString = $this->getClassString();			// String to embed in body, etc. element.
		$engine = strtolower($this->getEngine());		// Lower-case version of browser engine.
		$mobile = $this->isMobile();					// Is browser mobile?
		$name = strtolower($this->getBrowser());		// Lower-case version of browser name.
		$version = $this->getVersion();					// Browser version.
		
		$name = preg_replace("/internet explorer/i", "ie", $name);
	...
	* I now advocate usage of getClassString() inside appropriate elements like
	* the BODY and HTML.
	...
	*/
	...
	/**
	* Get a string for embedding inside HTML tags.
	* @param optional $prefix An optional prefix to prepend to the classname
	* Example Usage:		<body<?php echo $browser->getClassString(); ?>>
	* Result (Firefox):		<body class="firefox">
	* To Do:
	*	- Integrate _class_string property into class and related methods set..., get...
	*/
	public function getClassString($prefix = "", $compressIE = true, $addVersion = false, $fullVersion = false, $alwaysIncludeMain = true) {
		$name = strtolower($this->_browser_name);
		if ($compressIE)
			$name = str_replace("internet explorer", "ie", $name);
		...
		// Add both the overall class and version subclass if $
		if (!empty($ver) && $alwaysIncludeMain)
			$class = "$prefix$name $prefix$name$ver";
		else if (!empty($ver) && !$alwaysIncludeMain)
			$class = "$prefix$name$ver";
		else
			$class = "$prefix$name";
	...
	/**
	* Get the engine of the browser
	* @param $engine The name of the Browser engine
	*/
	public function getEngine() { return $this->_engine_name; }
	/**
	* Set the engine of the browser
	* @param $engine The name of the Browser engine
	*/
	public function setEngine($engine) { return $this->_engine_name = $engine; }
	/**
	...
	*/
	...
	/**
	 * Protected routine to determine the engine type
	 * @return boolean True if the browser was detected otherwise false
	 */
	 protected function checkEngines() {
		return (
			$this->checkEngineTrident() ||
			$this->checkEngineWebKit() ||
			$this->checkEngineGecko()
		);
	 }
	
	/**
	 * Protected routine to determine if the browser is mobile
	 * @return boolean True if the browser is mobile, otherwise false
	 */
	 ...
	 */
	protected function checkEngineGecko() {
		if( stripos($this->_agent,"Gecko") !== false) {
			$this->setEngine(self::ENGINE_GECKO);
			return true;
		}
		return false;
	}
	 ...
	 * @return boolean True if the browser is Gecko, otherwise false
	 */
	protected function checkEngineTrident() {
		if( stripos($this->_agent,"Trident") !== false) {
			$this->setEngine(self::ENGINE_TRIDENT);
			return true;
		}
		return false;
	}
	...
	/**
	 * Determine if the browser is WebKit or not
	 * @return boolean True if the browser is WebKit, otherwise false
	 */
	protected function checkEngineWebKit() {
		if( stripos($this->_agent,"WebKit") !== false) {
			$this->setEngine(self::ENGINE_WEBKIT);
			return true;
		}
		return false;
	}
